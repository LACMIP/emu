...
- Specimens placed in vials should also be immobilized with tissue paper, especially if the specimens are have are very delicate or could be damaged by rolling in the vial. Archival tissue paper should be tightly packed into the end of the vial, then a larger (unpacked) plug of tissue paper should be used to hold the specimen in place [example]({{ site.baseurl }}/assets/images/imls_rehousingvial1.jpg). Do not pack the vial so tightly with tissue paper that it could crush the fossil. Ideally, the fossil should be at least partially visible from the exterior of the vial. **The goal is to completely and gently immobilize the specimen such that can be easily retrieved with minimal risk in the future.**
